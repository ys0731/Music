					...
					<div id="bbs">
						<div id="blist">
							...
							<c:if test="${adminSongVo.startPage > adminSongVo.pageRange }">
		                       	<li style="display: inline-block"><a href='index.do?reqPage=${adminSongVo.startPage - 1 }&stype=${param.stype}&sval=${param.sval }&orderby=${param.orderby }&direct=${param.direct }'><<</a></li>
		                    </c:if>
		                    <c:forEach var="rp" begin="${adminSongVo.startPage }" step="1" end="${adminSongVo.endPage }">
		                        <li style="display: inline-block"><a href='index.do?reqPage=${rp }&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby }&direct=${param.direct }' <c:if test="${rp == adminSongVo.reqPage }">style='color: red;'</c:if>>${rp }</a></li>                        
		                    </c:forEach>
		                    <c:if test="${adminSongVo.endPage < adminSongVo.totPage }">
		                      	<li style="display: inline-block"><a href='index.do?reqPage=${adminSongVo.endPage + 1 }&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby }&direct=${param.direct }'>>></a></li>
		                    </c:if>
							</ul>
							<!-- //페이징 처리 -->
