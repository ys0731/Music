		...
		<div id="container">
			<div id="content">
				...
				<div class="con">
					...
					<div id="bbs">
						<div id="blist">
							<p><span><strong>총 ${adminAlbumVo.totCount }개</strong>  |  ${adminAlbumVo.reqPage }/${adminAlbumVo.totPage }페이지</span></p>
								...
								<tbody>
									...
									<c:forEach var="vo" items="${list }">
										<tr>
											...
											<td>${vo.no }</td>
											<td class="title"><a href="view.do?no=${vo.no }&reqPage=${adminAlbumVo.reqPage}&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby}&direct=${param.direct}">${vo.album }</a></td>
							...
							<!-- 페이징 처리 -->
							<ul class='page'>
							<c:if test="${adminAlbumVo.startPage > adminAlbumVo.pageRange }">
		                       	<li style="display: inline-block"><a href='index.do?reqPage=${adminAlbumVo.startPage - 1 }&stype=${param.stype}&sval=${param.sval }&orderby=${param.orderby }&direct=${param.direct }'><<</a></li>
		                    </c:if>
		                    <c:forEach var="rp" begin="${adminAlbumVo.startPage }" step="1" end="${adminAlbumVo.endPage }">
		                        <li style="display: inline-block"><a href='index.do?reqPage=${rp }&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby }&direct=${param.direct }' <c:if test="${rp == adminAlbumVo.reqPage }">style='color: red;'</c:if>>${rp }</a></li>                        
		                    </c:forEach>
		                    <c:if test="${adminAlbumVo.endPage < adminAlbumVo.totPage }">
		                      	<li style="display: inline-block"><a href='index.do?reqPage=${adminAlbumVo.endPage + 1 }&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby }&direct=${param.direct }'>>></a></li>
		                    </c:if>
							</ul>
							<!-- //페이징 처리 -->
							<form name="searchForm" id="searchForm" action="index.do"  method="post">
								<div class="search">
	                                ...
	                                <select id="stype" name="stype" class="dSelect" title="검색분류 선택">
	                                    ...
	                                    <option value="album"<c:if test="${param.stype == 'album'}">selected</c:if>>앨범명</option>
	                                </select>
