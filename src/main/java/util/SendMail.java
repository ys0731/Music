...
public class SendMail {
	...
	public static void sendMail(String from, String to, String subject, String content) {
		try {
			...
			prop.put("mail.smtp.ssl.protocols", "TLSv1.2"); //에러나는 경우 추가 TLS버전
			//prop.put("mail.smtp.ssl.trust", "smtp.naver.com");
			
			//2. Session 객체 생성
			Session session = Session.getDefaultInstance(prop, new Authenticator() {
				protected PasswordAuthentication getPasswordAuthentication() {
					return new PasswordAuthentication("lsmvip0522", "lsmsyyw0522^^"); //네이버 아이디 비밀번호 입력
				}
			});
			session.setDebug(true);
			
			//3. MimeMessage 객체 생셩(발신자, 수신자, 제목, 내용 설정)
